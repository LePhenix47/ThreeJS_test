...
export type DirectionalLightState = {
  color: string;
  intensity: number;
  positionX: number;
  positionY: number;
...
export type DirectionalLightUniformValue = {
  color: THREE.Color;
  intensity: number;
...
export type LightEntityParams = {
  id: number;
  parentFolder: GUI;
  defaults: DirectionalLightState;
  onChange: () => void;
  onRemove: (self: DirectionalLightEntity) => void;
};
 ...
 * with a future one.
 */
class DirectionalLightEntity implements Destroyable {
  public readonly id: number;
  private helper: DirectionalLightHelper;
  private registry: GUIStateRegistry<DirectionalLightState>;
  private folder: GUI;
  private readonly onChange: () => void;
  private readonly onRemove: (self: DirectionalLightEntity) => void;
  ...
  private setHelper(defaults: DirectionalLightState): void {
  ...
  private setRegistry(defaults: DirectionalLightState): void {
    const keyName = `shading-point-light-${this.id}`;
    this.registry = new GUIStateRegistry<DirectionalLightState>(
      keyName,
      defaults,
    );
  }
